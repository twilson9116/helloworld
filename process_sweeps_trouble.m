...
for jj=1:length(freq_uniques);                                                %
    ...
    if any([2:18]==freq_uniques(jj))
        freq_index=find(freq_uniques(jj)==frequency);
        % creates an array of the indecies where a frequency begins and starts- so this code will not work if frequency data is not grouped I am proud of this one
       ...
       opts.MaxIter=8000;
        ...
        %check Goodness of Fit
         ...
         if best_gof<gof_setting;
            ...
            %pause 
             k1_array = k1_guess.*[1 -100 100 1e3 -1e3 -1e4];
             k2_array = k2_guess.*[10 100 1e3 -1e3 -1e4];
