...
M_low=confidence_f_v_Hr_coeffs(1,1);
g_high=confidence_f_v_Hr_coeffs(2,2);
g_low=confidence_f_v_Hr_coeffs(1,2);
%plot
...
output_folder=horzcat('Out',input_file,'_',timestamp); %name of output file
mkdir(output_folder);

%Create file
output_file=horzcat('RESULT','_',input_file,timestamp,'.dat'); % <- name for output data file
fileID=fopen([output_file],'w');
results=[M gamma alpha dH_intercept];
results_low=[M_low g_low alpha_low dH_intercept_low];
results_high=[M_high g_high alpha_high dH_intercept_high];
%fprintf(fileID','%14s\r\n','Top row= results, 2nd row=low, 3rd row=high')
fprintf(fileID,'%12s %12s %12s %12s\r\n','M(Oe)', '\Gamma(GHZ/Oe)', 'alpha()', 'dH_0(Oe)');
