...
for jj=1:length(freq_uniques);                                                %
    ...
    if any([2:18]==freq_uniques(jj))
        ...
        try
            ...
            [fitresult,GOF_out]=fit_data(H_chopped,R_chopped,GOF,Hr_2,dH_2,k1_guess2,k2_guess2);
            H_res=fitresult.Hr;
            delta_H=fitresult.delta_H;
            
            fighand=figure('Name',['freq=' num2str(freq_uniques(jj))
            plot(fitresult,current_H_data,current_R_data)
            hold
    ...
    [H_prepared,Amplitude_prepared]=prepareCurveData(current_H_data,Amplitude_in);
        ...
        %check Goodness of Fit
         ...
         if best_gof<gof_setting;
             k1_array = k1_guess*[-10 100 -100 1000 -1000];
