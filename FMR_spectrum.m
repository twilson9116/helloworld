...
function H_Table=FMR_spectrum(input,plot_sweeps,Use_rigorous_fit)
...
plotsfolder=horzcat('plots_',[datestr(now,'mmdd_HH_MM')])
if plot_sweeps
    mkdir(plotsfolder);
end
...
if isdir(input);
    ...
    for j=1:length(file_list);
        ...
        if plot_sweeps
            figs=get(0,'children');
            for k=1:length(figs)
                saveas(figs(k),[plotsfolder '\' figs(k).Name])
            end
...
for i=1:length(unique_f);
    try
        ...
        if Use_rigorous_fit;
            ...
            R_to_fit=current_R_data(indexlow:indexup);
            H_to_fit=current_H_data(indexlow:indexup);
        else
