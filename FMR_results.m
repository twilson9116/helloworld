...
%% PLOTS
if plots_true_false
fighand=figure('Name', ['f vs Hr' Sample_list{i}]) ;
plot(kfit,Hr,f);
grid on; legend off; xlabel( 'Hr' ); ylabel( 'frequency' );

fighand=figure('Name',['dH vs f for' Sample_list{i}]);
hold on
plot(dHfit,f,dH);
...
if plots_true_false
    plotsfolder=['Results_plots_' datestr(now,'mmdd_HH_MM')]
    mkdir(plotsfolder)
    figs=get(0,'children');
for i=1:length(figs)
    saveas(figs(i),[plotsfolder '\' figs(i).Name(1:end-22)]);
end
end
